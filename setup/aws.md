...
4. In a minute you should have finished creating your database.  Navigate to `Connectivity & security` and copy the `Endpoint` URL like so:
...
6. Fill the following configurations for your new connection, supplying the username and password configured in AWS.  Instead of `localhost` your **Host:** will be the pasted endpoint from AWS.

<img src="./images/pg-connect.png" width="600"/>

7. Click `Test Connection` and if it's successful you should see a message that says `Connected` and will list the server it's running on along with the driver. Click `Finish`.
